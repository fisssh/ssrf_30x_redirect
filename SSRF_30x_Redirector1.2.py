...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>云原生穿透引擎 · K8s/Docker 307无损引流 · Gopher封装 · 批量队列投递 · 全局高级请求编辑</p>
        </div>
        <div class="stats">
            ...
            <div class="card">
                ...
                <div id="tab-basic" class="tab-content active">
                    ...
                    <div class="req-editor" id="basicReqEditor" style="display:none;">
                        ...
                        <button class="btn btn-small" style="margin-top:8px;" onclick="applyBasicCustomRequest()">✅ 生成 Gopher Payload</button>
                    </div>
                ...
                <div id="tab-stateful" class="tab-content">
                    ...
                    <div class="form-group" style="margin-top:20px;" id="finalTargetGroup" hidden>
                        ...
                        <div class="req-editor" id="statefulReqEditor" style="display:none;">
                            ...
                            <label>请求体</label>
                            <textarea class="req-body" placeholder="JSON或任意格式"></textarea>
                            <button class="btn btn-small" style="margin-top:8px;" onclick="applyStatefulCustomRequest()">✅ 生成 Gopher Payload</button>
                        </div>
                    </div>
    ...
    <script>
        ...
        function applyBasicCustomRequest() {
            ...
            if (gopherUrl) { document.getElementById('targetUrl').value = gopherUrl; editor.style.display = 'none'; alert('✅ Gopher Payload 已生成'); }
        }

        function applyStatefulCustomRequest() {
            const editor = document.getElementById('statefulReqEditor');
            const gopherUrl = buildGopherFromEditor(editor);
            if (gopherUrl) { document.getElementById('statefulFinalTarget').value = gopherUrl; editor.style.display = 'none'; alert('✅ Gopher Payload 已生成'); }
        }

        function toggleBasicEditor() { const e = document.getElementById('basicReqEditor'); e.style.display = e.style.display === 'none' ? 'block' : 'none'; }
        function toggleStatefulEditor() { const e = document.getElementById('statefulReqEditor'); e.style.display = e.style.display === 'none' ? 'block' : 'none'; }
        ...
        async function refreshLogs() {
            ...
            try {
                ...
                box.innerHTML = visibleLogs.map(l => {
                    ...
                    return `
                    <div class="log-entry" data-logid="${l.id}" onclick="const ext=this.querySelector('.log-ext');ext.style.display=ext.style.display==='none'?'block':'none';">
                        <button class="delete-log-btn" onclick="event.stopPropagation();hideLog('${l.id}')" title="隐藏">✕</button>
                        <div class="log-header">
                            <span class="log-ip">${l.ip}</span>
                            <span><span class="method-badge ${mClass}">${l.method}</span><span style="color:var(--text-muted);font-size:12px;">${l.timestamp}</span></span>
                        </div>
...
def create_stateful_chain():
    ...
    STATEFUL_CHAINS[chain_id] = {
        ...
        'targets_queue': [],
        'queue_index': 0,
        'queue_mode': 'sequential'  # 'sequential' or 'loop'
    }
