...
<body>
        ...
        <div class="grid">
            <div class="card">
                ...
                <div class="nav-tabs">
                    ...
                    <button class="nav-tab" data-tab="loop">循环实验</button>
                </div>
                ...
                <div id="tab-loop" class="tab-content">
                    <div class="protocol-tip tip-orange">
                        <b>🔁 有界循环重定向实验说明：</b>同源 <code style="color:var(--accent-orange);">A↔B</code> 乒乓循环跳转 (URL 完全重复, 计数在服务端),
                        用于观测 HTTP 客户端 (浏览器 / 服务端 fetch 库) 面对循环重定向的真实行为:<br>
                        ① 是否跟随 <b>http 30x / meta 刷新 / js 跳转</b>三种方式 (meta/js 可鉴别渲染型客户端: 爬虫 / 无头浏览器);
                        ② 实际跟随几跳 (对比客户端 max_redirects 上限);
                        ③ <b>300/301/302/303 方法改写谱系 vs 307/308 保留方法与请求体, 305 是否被忽略</b> (SSRF 打内网 API 的关键前提);
                        ④ 是否检测 URL 循环 (实测 requests / curl / 主流浏览器均只数跳数, 不识别 A↔B 重复)。<br>
                        <b>盲SSRF用法：</b>将目标 SSRF 点指向生成的 <code style="color:var(--accent-orange);">/loop/start</code> 地址, 用 <code style="color:var(--accent-orange);">tag</code> 区分注入点 ——
                        即使无回显, 右侧日志时间线与下方会话列表 (含逐跳捕获明细) 即为带外证据; 用 307+POST 可验证请求体能否穿透重定向到达内网目标。
                    </div>

                    <!-- v2.7: 流程原理图 (内嵌 SVG, 与 /loop 控制页浅色版同源) -->
                    <div style="background:var(--bg-secondary);border:1px solid var(--border-color);border-radius:10px;padding:14px 16px;margin-bottom:16px;">
                        <div style="font-size:13px;font-weight:600;color:var(--text-primary);margin-bottom:6px;">🗺 流程原理图</div>
                        <svg viewBox="0 0 760 258" style="width:100%;height:auto;display:block;" role="img" aria-label="循环实验流程原理图">
                            <defs><marker id="loopArrD" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M2 1L8 5L2 9" fill="none" stroke="#8b949e" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></marker></defs>
                            <rect x="18" y="18" width="150" height="52" rx="8" fill="#161b22" stroke="#58a6ff"/>
                            <text x="93" y="38" text-anchor="middle" font-size="13" font-weight="600" fill="#58a6ff">客户端</text>
                            <text x="93" y="56" text-anchor="middle" font-size="11" fill="#8b949e">浏览器 / 服务端 fetch 库</text>
                            <rect x="248" y="18" width="212" height="52" rx="8" fill="#161b22" stroke="#39d5ff"/>
                            <text x="354" y="38" text-anchor="middle" font-size="13" font-weight="600" fill="#39d5ff">/loop/start</text>
                            <text x="354" y="56" text-anchor="middle" font-size="11" fill="#8b949e">建会话+首跳 ?mode/code/limit/tag</text>
                            <rect x="540" y="18" width="202" height="52" rx="8" fill="none" stroke="#8b949e" stroke-width="0.8" stroke-dasharray="5 4"/>
                            <text x="641" y="38" text-anchor="middle" font-size="12" font-weight="600" fill="#e6edf3">三种跳转方式 mode</text>
                            <text x="641" y="56" text-anchor="middle" font-size="11" fill="#8b949e">http 30x · meta 刷新 · js 跳转</text>
                            <line x1="168" y1="44" x2="242" y2="44" stroke="#8b949e" stroke-width="1.5" marker-end="url(#loopArrD)"/>
                            <text x="205" y="34" text-anchor="middle" font-size="11" fill="#8b949e">1 请求</text>
                            <rect x="200" y="118" width="130" height="52" rx="8" fill="#161b22" stroke="#f0883e"/>
                            <text x="265" y="138" text-anchor="middle" font-size="13" font-weight="600" fill="#f0883e">/loop/a</text>
                            <text x="265" y="156" text-anchor="middle" font-size="11" fill="#8b949e">乒乓节点 A</text>
                            <rect x="400" y="118" width="130" height="52" rx="8" fill="#161b22" stroke="#f0883e"/>
                            <text x="465" y="138" text-anchor="middle" font-size="13" font-weight="600" fill="#f0883e">/loop/b</text>
                            <text x="465" y="156" text-anchor="middle" font-size="11" fill="#8b949e">乒乓节点 B</text>
                            <text x="365" y="106" text-anchor="middle" font-size="11" fill="#f0883e">A↔B 互指 · URL 完全重复</text>
                            <line x1="330" y1="136" x2="394" y2="136" stroke="#8b949e" stroke-width="1.5" marker-end="url(#loopArrD)"/>
                            <line x1="400" y1="154" x2="336" y2="154" stroke="#8b949e" stroke-width="1.5" marker-end="url(#loopArrD)"/>
                            <rect x="18" y="118" width="150" height="52" rx="8" fill="#161b22" stroke="#8b949e" stroke-width="0.8"/>
                            <text x="93" y="138" text-anchor="middle" font-size="12" font-weight="600" fill="#e6edf3">服务端计数+逐跳捕获</text>
                            <text x="93" y="156" text-anchor="middle" font-size="11" fill="#8b949e">方法·体长·CT·SHA</text>
                            <line x1="168" y1="144" x2="194" y2="144" stroke="#8b949e" stroke-width="0.8" stroke-dasharray="4 3"/>
                            <polyline points="354,70 354,92 265,92 265,112" fill="none" stroke="#8b949e" stroke-width="1.5" marker-end="url(#loopArrD)"/>
                            <text x="372" y="88" font-size="11" fill="#8b949e">2 首跳</text>
                            <line x1="265" y1="170" x2="265" y2="204" stroke="#8b949e" stroke-width="1.5" marker-end="url(#loopArrD)"/>
                            <text x="277" y="192" font-size="11" fill="#8b949e">3 达到 limit</text>
                            <rect x="200" y="208" width="330" height="44" rx="8" fill="#161b22" stroke="#3fb950"/>
                            <text x="365" y="226" text-anchor="middle" font-size="13" font-weight="600" fill="#3fb950">200 JSON 终止 · 服务端熔断</text>
                            <text x="365" y="243" text-anchor="middle" font-size="11" fill="#8b949e">观测: 终端逐跳JSON · 日志时间线 · 会话进度条/逐跳明细</text>
                        </svg>
                    </div>
                    ...
                    <div class="bb-flow-steps">
                        <div class="bb-flow-step">
                            <div class="bb-flow-num">1</div>
                            <div><b>配置实验参数</b><span>跳转方式 / 状态码 / 重定向次数 (服务端熔断线) / 测试标记</span></div>
                        </div>
                        <div class="bb-flow-step">
                            <div class="bb-flow-num">2</div>
                            <div><b>指向 SSRF 注入点</b><span>将生成的 /loop/start 地址作为 payload; 或用高级请求编辑自定义首跳后直发/生成 curl</span></div>
                        </div>
                        <div class="bb-flow-step">
                            <div class="bb-flow-num">3</div>
                            <div><b>观测逐跳行为</b><span>日志时间线 / 活跃会话列表: 跟随跳数 · 逐跳方法与请求体捕获 · 循环检测 · 熔断终止</span></div>
                        </div>
                    </div>
                    ...
                    <div class="bb-create-grid four">
                        <div>
                            <label>跳转方式 mode</label>
                            <select id="loopMode" onchange="loopModeChanged()">
                                <option value="http" selected>http · 30x 状态码跳转</option>
                                <option value="meta">meta · 刷新 (200+HTML)</option>
                                <option value="js">js · 跳转 (200+Script)</option>
                            </select>
                        </div>
                        <div>
                            <label>状态码 (仅 http 方式生效)</label>
                            <select id="loopCode">
                                <option value="300">300 · 多选 (通常不自动跟随)</option>
                                <option value="301">301 · 永久 (POST 常改 GET)</option>
                                <option value="302" selected>302 · 临时 (POST 常改 GET)</option>
                                <option value="303">303 · 强制改写为 GET</option>
                                <option value="305">305 · 代理 (现代客户端忽略)</option>
                                <option value="307">307 · 保留方法与请求体</option>
                                <option value="308">308 · 永久 + 保留方法与请求体</option>
                            </select>
                        </div>
                        <div>
                            <label>重定向次数 (1-40)</label>
                            <input type="text" id="loopLimit" value="12" placeholder="12">
                        </div>
                        <div>
                            <label>测试标记 tag (区分注入点)</label>
                            <input type="text" id="loopTag" value="ssrf-test" placeholder="ssrf-test" maxlength="64">
                        </div>
                    </div>

                    <div style="display:flex;gap:10px;flex-wrap:wrap;margin:4px 0 16px;">
                        <button class="btn" onclick="generateLoopPayload()">🔁 生成实验 Payload</button>
                        <button class="btn btn-cyan btn-small" onclick="window.open(BASE_URL + '/loop', '_blank')">🧪 打开实验控制页 (/loop)</button>
                        <button class="btn btn-green btn-small" onclick="doCopy(BASE_URL + '/loop/ok', '✅ 基线对照地址已复制')">📋 复制基线对照 /loop/ok</button>
                        <button class="btn btn-warning btn-small" onclick="toggleLoopEditor()">✏️ 高级请求编辑</button>
                    </div>

                    <!-- v2.7: 高级请求编辑 (自定义首跳 方法/请求头/请求体 → curl / 浏览器直发) -->
                    <div id="loopAdvEditor" style="display:none;background:var(--bg-card);border:1px solid var(--border-color);border-radius:10px;padding:14px 16px;margin-bottom:16px;">
                        <div style="font-size:12px;color:var(--text-secondary);line-height:1.8;margin-bottom:12px;">
                            自定义发往 <code>/loop/start</code> 的<b>首跳请求</b> (方法 / 请求头 / 请求体), 配合 307/308 验证方法与请求体能否穿透全链 ——
                            每一跳实际到达的 方法·体长·Content-Type·体SHA·预览 都会在下方会话明细中逐跳展开。<br>
                            ① <b>生成 curl 命令</b>: 投放到任意可执行环境, 用指定客户端复现整条链路;
                            ② <b>浏览器直发</b>: 用当前浏览器 fetch 跑一遍 (自定义请求头在同源跳转中会被自动重发; 非简单头会触发 CORS 预检 OPTIONS, 不消耗计数)。
                        </div>
                        <div class="bb-create-grid" style="grid-template-columns:160px 1fr;">
                            <div>
                                <label>请求方法</label>
                                <select id="loopAdvMethod">
                                    <option value="GET">GET</option>
                                    <option value="POST" selected>POST</option>
                                    <option value="PUT">PUT</option>
                                    <option value="PATCH">PATCH</option>
                            ...
                            <div>
                                <label>请求体 (GET / HEAD 自动忽略)</label>
                                <textarea id="loopAdvBody" rows="3" style="width:100%;" placeholder='{"username":"admin","password":"***"} 或任意格式' spellcheck="false"></textarea>
                            </div>
                        </div>
                        <label>自定义请求头</label>
                        <div class="req-headers" id="loopReqHeaders"></div>
                        <button class="btn btn-ghost btn-small" onclick="addHeaderRow('loopReqHeaders')" style="margin-bottom:12px;">+ 添加请求头</button>
                        <div style="display:flex;gap:10px;flex-wrap:wrap;">
                            <button class="btn btn-small" onclick="generateLoopCurl()">📋 生成并复制 curl 命令</button>
                            <button class="btn btn-cyan btn-small" onclick="sendLoopCustomRequest()">🚀 浏览器直发测试</button>
                        </div>
                    </div>

                    <div style="background:var(--bg-card);border:1px solid var(--border-color);border-left:4px solid var(--accent-orange);border-radius:8px;padding:12px 16px;font-size:12px;color:var(--text-secondary);line-height:1.9;margin-bottom:4px;">
                        <b style="color:var(--text-primary);">💡 设计要点：</b>
                        计数器存服务端, /loop/a 与 /loop/b 两个 URL 完全不变地交替 —— 专门制造「真循环」测试客户端循环检测能力;
                        循环防护 = 客户端跳数上限 ∪ 服务端硬熔断 (最多 40 次, 会话 5 分钟过期), 两者缺一都可能出事;
                        OPTIONS 探测不消耗计数、不进捕获, 避免污染方法保留实验数据;
                        逐跳捕获 (方法/体长/CT/体SHA/预览) 仅存会话内存供控制台展示, 终端 JSON 日志不含请求体。
                    </div>

                    <!-- 活跃会话面板 -->
                    <div class="chain-manager" style="margin-top:18px;">
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:14px; flex-wrap:wrap; gap:8px;">
                            <h3>🧪 活跃实验会话 (<span id="loopSessionCount">0</span>)</h3>
                            <button class="manual-refresh-btn" onclick="loadLoopSessions()">🔄 刷新</button>
                        </div>
                        <div style="font-size:12px;color:var(--text-secondary);margin-bottom:12px;line-height:1.7;">
                            每个会话 = 一次进行中的循环实验 (5 分钟过期自动回收)。「进度」实时反映客户端已跟随的跳数 ——
                            盲SSRF场景下, 进度条增长即目标服务器正在跟随重定向的带外证据; 展开「逐跳捕获明细」可核对每一跳的方法与请求体去向。
                        </div>
                        <div id="loopSessionContainer">
                            <div class="no-chains-msg">暂无活跃实验会话</div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function pbChainTypeName(pathname) {
            ...
            if (pathname === '/bb' || pathname.startsWith('/bb/')) return '盲测端点';
            if (pathname === '/loop' || pathname.startsWith('/loop/')) return '循环实验';
            return '有状态链';
        }
        ...
        document.querySelectorAll('.nav-tab[data-tab]').forEach(tab => {
            tab.addEventListener('click', function() {
                ...
                if (this.dataset.tab === 'loop') loadLoopSessions();
            });
        });
        ...
        async function refreshLogs() {
            ...
            try {
                ...
                box.innerHTML = visibleLogs.map(l => {
                    ...
                    const loopData = (l.extra_data && (l.extra_data.loop_redirect || l.extra_data.loop_finished)) ? l.extra_data : null;
                    ...
                    const loopBadge = loopData ? `<span style="background:rgba(240,136,62,0.14);color:var(--accent-orange);padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;margin-left:6px;">🔁循环:${loopData.redirects}跳${loopData.loop_finished ? '·完成' : ''}${loopData.tag ? '·' + escapeHtml(String(loopData.tag)) : ''}</span>` : '';
                    return `
                    <div class="log-entry ${headBypassClass}" data-logid="${l.id}" onclick="const ext=this.querySelector('.log-ext');ext.style.display=ext.style.display==='none'?'block':'none';">
                        ...
                        <div class="log-header">
                            ...
                            <span><span class="method-badge ${mClass}">${safeMethod}</span>${headBadge}${fbBadge}${blindBadge}${loopBadge}<span style="color:var(--text-muted);font-size:12px;">${safeTime}</span></span>
                        </div>
        ...
        // ========== v2.7: 有界循环重定向实验 (三种跳转方式 + 高级请求编辑 + 逐跳捕获) ==========
        function loopModeChanged() {
            document.getElementById('loopCode').disabled = document.getElementById('loopMode').value !== 'http';
        }

        function toggleLoopEditor() {
            const el = document.getElementById('loopAdvEditor');
            el.style.display = (el.style.display === 'none' || !el.style.display) ? 'block' : 'none';
        }

        function buildLoopStartUrl() {
            const mode = document.getElementById('loopMode').value;
            const code = document.getElementById('loopCode').value;
            const limitRaw = document.getElementById('loopLimit').value.trim();
            const tag = document.getElementById('loopTag').value.trim();
            const limit = parseInt(limitRaw, 10);
            if (isNaN(limit) || limit < 1 || limit > 40) { alert('⚠️ 重定向次数必须为 1-40 的整数 (服务端熔断线)'); return null; }
            if (tag && !/^[A-Za-z0-9_-]{1,64}$/.test(tag)) { alert('⚠️ 测试标记 tag 仅支持 字母/数字/_/-, 最长 64 字符'); return null; }
            let url = `${BASE_URL}/loop/start?limit=${limit}&mode=${mode}`;
            if (mode === 'http') url += `&code=${code}`;
            if (tag) url += `&tag=${encodeURIComponent(tag)}`;
            return url;
        }

        function generateLoopPayload() {
            const url = buildLoopStartUrl();
            if (url) showResult(url);
        }

        // v2.7: 高级请求编辑 —— 收集自定义 方法/请求头/请求体
        function collectLoopAdvRequest() {
            const method = document.getElementById('loopAdvMethod').value;
            const body = document.getElementById('loopAdvBody').value;
            const headers = {};
            document.querySelectorAll('#loopReqHeaders .header-row').forEach(r => {
                const k = r.querySelector('.header-key').value.trim();
                if (k) headers[k] = r.querySelector('.header-value').value;
            });
            return { method, headers, body: (method === 'GET' || method === 'HEAD') ? '' : body };
        }

        function shellSingleQuote(s) { return "'" + String(s).replace(/'/g, "'\\''") + "'"; }

        function generateLoopCurl() {
            const url = buildLoopStartUrl();
            if (!url) return;
            const adv = collectLoopAdvRequest();
            const limit = parseInt(document.getElementById('loopLimit').value.trim(), 10) || 12;
            let cmd = `curl -i -L --max-redirs ${Math.min(limit + 2, 45)} -X ${adv.method}`;
            for (const k in adv.headers) cmd += ' -H ' + shellSingleQuote(k + ': ' + adv.headers[k]);
            if (adv.body) cmd += ' --data ' + shellSingleQuote(adv.body);
            cmd += ' ' + shellSingleQuote(url);
            doCopy(cmd, '✅ curl 命令已复制 (已含自定义方法/请求头/请求体)!');
        }

        async function sendLoopCustomRequest() {
            const url = buildLoopStartUrl();
            if (!url) return;
            const adv = collectLoopAdvRequest();
            const opt = { method: adv.method, headers: adv.headers, redirect: 'follow' };
            if (adv.body) opt.body = adv.body;
            try {
                const res = await fetch(url, opt);
                const text = await res.text();
                let extra = '';
                try { const j = JSON.parse(text); if (j.redirects !== undefined) extra = ` · 完成 ${j.redirects} 跳 · 最终方法 ${j.final_method}`; } catch(e) {}
                alert(`🚀 浏览器直发完成: HTTP ${res.status}${extra}\n逐跳捕获明细见下方「活跃实验会话」`);
                loadLoopSessions();
            } catch(e) { alert('❌ 请求失败: ' + e); }
        }

        async function loadLoopSessions() {
            try {
                const res = await fetch(CONSOLE_PREFIX + '/api/loop_sessions');
                const data = await res.json();
                const container = document.getElementById('loopSessionContainer');
                const sessions = data.sessions || [];
                document.getElementById('loopSessionCount').textContent = sessions.length;
                if (sessions.length === 0) { container.innerHTML = '<div class="no-chains-msg">暂无活跃实验会话</div>'; return; }
                const modeNames = { http: '30x', meta: 'META', js: 'JS' };
                container.innerHTML = sessions.map(s => {
                    const pct = s.limit > 0 ? Math.min(100, Math.round((s.redirects / s.limit) * 100)) : 0;
                    const done = s.redirects >= s.limit;
                    const barColor = done ? 'var(--accent-green)' : 'var(--accent-orange)';
                    const mode = s.mode || 'http';
                    const startUrl = `${BASE_URL}/loop/start?limit=${s.limit}&mode=${mode}${mode === 'http' ? '&code=' + s.status : ''}${s.tag ? '&tag=' + encodeURIComponent(s.tag) : ''}`;
                    const safeTag = s.tag ? escapeHtml(String(s.tag)) : '(无标记)';
                    const doneBadge = done ? '<span style="background:rgba(63,185,80,0.14);color:var(--accent-green);padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;margin-left:6px;">已熔断</span>' : '';
                    const modeBadge = `<span style="background:rgba(57,213,255,0.14);color:var(--accent-cyan);padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;margin-left:6px;">${modeNames[mode] || escapeHtml(String(mode))}</span>`;
                    const codeBadge = mode === 'http' ? `<span style="background:rgba(240,136,62,0.14);color:var(--accent-orange);padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;margin-left:6px;">${s.status}</span>` : '';
                    const methodLine = s.last_method ? ' · 最近方法: ' + escapeHtml(String(s.last_method)) : '';
                    const hops = s.hops || [];
                    const hopRows = hops.map(h => `<tr>`
                        + `<td style="padding:3px 8px;">#${h.n}</td>`
                        + `<td style="padding:3px 8px;color:var(--accent-cyan);font-weight:600;">${escapeHtml(String(h.method))}</td>`
                        + `<td style="padding:3px 8px;">${escapeHtml(String(h.path))}</td>`
                        + `<td style="padding:3px 8px;">${escapeHtml(h.ct || '-')}</td>`
                        + `<td style="padding:3px 8px;">${h.blen}</td>`
                        + `<td style="padding:3px 8px;font-family:monospace;">${h.bsha ? escapeHtml(h.bsha) : '-'}</td>`
                        + `<td style="padding:3px 8px;color:var(--text-muted);word-break:break-all;">${h.bprev ? escapeHtml(h.bprev) : ''}</td>`
                        + `</tr>`).join('');
                    const hopsHtml = hops.length ? `<details style="margin-top:8px;"><summary style="cursor:pointer;font-size:12px;color:var(--accent);">逐跳捕获明细 (${hops.length}) — 方法 · 路径 · Content-Type · 体长 · 体SHA · 预览</summary>`
                        + `<div style="overflow-x:auto;margin-top:6px;"><table style="width:100%;border-collapse:collapse;font-size:11px;color:var(--text-secondary);">`
                        + `<thead><tr style="text-align:left;color:var(--text-muted);border-bottom:1px solid var(--border-color);"><th style="padding:3px 8px;">跳</th><th style="padding:3px 8px;">方法</th><th style="padding:3px 8px;">路径</th><th style="padding:3px 8px;">Content-Type</th><th style="padding:3px 8px;">体长</th><th style="padding:3px 8px;">体SHA</th><th style="padding:3px 8px;">预览</th></tr></thead>`
                        + `<tbody>${hopRows}</tbody></table></div></details>` : '';
                    return `
                    <div class="chain-item">
                        <div class="chain-item-header">
                            <span class="chain-item-id">🔁 ${escapeHtml(String(s.token_short))} ${modeBadge}${codeBadge}${doneBadge}</span>
                            <span class="chain-item-time">标记: ${safeTag} · 剩余 ${s.ttl_remaining}s</span>
                        </div>
                        <div style="font-size:13px;color:var(--text-muted);margin:6px 0 8px;">已跟随 <span style="color:${barColor};font-weight:700;">${s.redirects}</span> / ${s.limit} 跳${methodLine}</div>
                        ...
                        <div class="chain-item-actions">
                            <button class="chain-edit-btn chain-copy-btn" onclick="doCopy('${startUrl}', '✅ 已复制同类实验地址!')">📋 复制同类地址</button>
                            <button class="chain-edit-btn chain-delete-btn" onclick="deleteLoopSession('${s.token}')">🗑 终止会话</button>
                        </div>
                    </div>`;
                }).join('');
            } catch(e) { console.error('加载循环实验会话失败:', e); }
        }

        async function deleteLoopSession(token) {
            if (!confirm('确定终止该实验会话? 客户端后续跳转将收到 410')) return;
            try {
                const res = await fetch(`${CONSOLE_PREFIX}/api/loop_sessions/${token}`, { method: 'DELETE' });
                const data = await res.json();
                if (data.error) alert('❌ ' + data.error); else loadLoopSessions();
            } catch(e) { alert('❌ 网络错误'); }
        }
...
def create_stateful_chain():
    ...
    for step in steps_input:
        ...
        # v2.5: /loop 前缀为有界循环重定向实验命名空间, 同样保留防止链路径遮蔽实验路由
        reserved = ['/r', '/c']
        if (path in reserved or path == CONSOLE_PATH or path == CONSOLE_PATH + '/'
                ...
                or path == '/loop' or path.startswith('/loop/')):
...
# ==================== v2.7: 有界循环重定向实验模块 (Redirect Loop Lab) ====================
# 整合自独立实验脚本 redirect_loop_lab.py —— 同源 A<->B 有界循环重定向实验环境
#
# 端点一览:
#   GET  /loop            实验控制页 (浏览器测试入口, 内嵌流程原理图)
#   ANY  /loop/start      创建实验会话并发出第一跳  ?limit=12&code=302&mode=http&tag=xxx
#   ANY  /loop/a          乒乓跳转节点 A (与 B 的 URL 完全重复, 计数在服务端)
#   ANY  /loop/b          乒乓跳转节点 B
#   ANY  /loop/ok         基线对照端点 (验证客户端能抓取普通 URL; 不占用根级 /ok, 避免与有状态链路径冲突)
#
# v2.7 增强:
#   - 更多重定向方式 (mode): http (30x 状态码) / meta (200 + meta refresh) / js (200 + JS location),
#     用于区分纯 HTTP 客户端与渲染型客户端 (爬虫/无头浏览器常出现在 SSRF 渲染链中);
#   - 更多状态码 (code): 300/301/302/303/305/307/308 七档, 覆盖 方法改写/保留/忽略 全谱系;
#   - 逐跳请求捕获: 每跳记录 方法/路径/Content-Type/体长/体SHA/预览, 控制台会话明细可展开,
#     直观呈现 302/303 改写与 307/308 穿透过程中请求体在每一跳的去向;
#   - 高级请求编辑 (控制台): 自定义首跳 方法/请求头/请求体, 一键生成等价 curl 命令或浏览器直发;
#   - 控制台「循环实验」Tab 与 /loop 控制页均内嵌流程原理图 (SVG)。
#
# 实验目标 —— 观测 HTTP 客户端 (浏览器 / 服务端 fetch 库) 面对循环重定向的真实行为:
#   1. 是否跟随 http 30x / meta / js 三种跳转方式;
#   2. 实际跟随几跳 (对比客户端自身 max_redirects 上限);
#   3. 300/301/302/303 是否将 POST 改写为 GET, 307/308 是否保留方法与请求体, 305 是否被忽略
#      (SSRF 打内网 API 的关键前提);
#   4. 客户端是否检测 URL 循环 (实测 requests/curl/主流浏览器均只数跳数, 不识别 A<->B 重复)
#
# 设计要点:
#   - 计数器存服务端 (LOOP_SESSIONS), /loop/a 与 /loop/b 两个 URL 完全不变地交替,
#     专门制造"真循环"以测试客户端的循环检测能力;
#   - limit 硬上限 LOOP_MAX_LIMIT: 即使客户端未设 max_redirects, 服务端也能熔断终止
#     (循环防护 = 客户端跳数上限 ∪ 服务端硬熔断, 两者缺一都可能出事);
#   - OPTIONS 探测不消耗计数、不进逐跳捕获 (避免污染方法保留实验数据);
#   - 逐跳日志双通道: 服务端终端 JSON 行 + Web 控制台日志时间线 (log_request);
#     终端 JSON 不含请求体/Cookie/Authorization; 逐跳捕获的请求体预览仅存会话内存,
#     供控制台明细展示, 不落终端日志;
#   - 盲SSRF用法: 将目标 SSRF 点指向 /loop/start?tag=注入点标识, 即使无回显,
#     终端与控制台日志即为带外证据; 307+POST 可验证请求体能否穿透重定向;
#   - 公开负载端点不校验 access_token (与 /r /c /bb 同策略, 供 SSRF 目标直接访问);
#   - 会话存储与 STATEFUL_CHAINS / BLIND_BENCH 完全隔离, 互不影响。

import threading
import hashlib
from urllib.parse import urlencode as _loop_urlencode

LOOP_LOCK = threading.Lock()
LOOP_SESSIONS = {}            # token -> {limit, status, mode, tag, redirects, expires, hops, last_method}
LOOP_TTL = 300                # 实验会话有效期 (秒)
LOOP_MAX_SESSIONS = 1000      # 并发会话上限 (防自身被反向DoS)
LOOP_MAX_LIMIT = 40           # 单次实验最大重定向次数 (服务端熔断线)
LOOP_ALLOWED_CODES = (300, 301, 302, 303, 305, 307, 308)   # v2.7: 七档状态码全谱系
LOOP_ALLOWED_MODES = ('http', 'meta', 'js')                # v2.7: 三种跳转方式
LOOP_MAX_HOPS_CAPTURED = 50   # 每会话最多保存的逐跳快照数 (limit<=40 + 首跳/熔断末跳余量)
LOOP_BODY_PREVIEW = 160       # 逐跳请求体预览最大字符 (仅控制台展示, 不落终端日志)

# v2.7: 流程原理图 (浅色版, 内嵌于 /loop 控制页; 控制台 Tab 使用深色版)
LOOP_FLOW_SVG_LIGHT = r"""<svg viewBox="0 0 760 258" style="width:100%;height:auto;display:block;margin:6px 0 10px;" role="img" aria-label="循环实验流程原理图">
<defs><marker id="loopArrL" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M2 1L8 5L2 9" fill="none" stroke="#57606a" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></marker></defs>
<rect x="18" y="18" width="150" height="52" rx="8" fill="#f6f8fa" stroke="#0969da"/>
<text x="93" y="38" text-anchor="middle" font-size="13" font-weight="600" fill="#0969da">客户端</text>
<text x="93" y="56" text-anchor="middle" font-size="11" fill="#57606a">浏览器 / 服务端 fetch 库</text>
<rect x="248" y="18" width="212" height="52" rx="8" fill="#f6f8fa" stroke="#1a7f37"/>
<text x="354" y="38" text-anchor="middle" font-size="13" font-weight="600" fill="#1a7f37">/loop/start</text>
<text x="354" y="56" text-anchor="middle" font-size="11" fill="#57606a">建会话+首跳 ?mode/code/limit/tag</text>
<rect x="540" y="18" width="202" height="52" rx="8" fill="none" stroke="#57606a" stroke-width="0.8" stroke-dasharray="5 4"/>
<text x="641" y="38" text-anchor="middle" font-size="12" font-weight="600" fill="#1f2328">三种跳转方式 mode</text>
<text x="641" y="56" text-anchor="middle" font-size="11" fill="#57606a">http 30x · meta 刷新 · js 跳转</text>
<line x1="168" y1="44" x2="242" y2="44" stroke="#57606a" stroke-width="1.5" marker-end="url(#loopArrL)"/>
<text x="205" y="34" text-anchor="middle" font-size="11" fill="#57606a">1 请求</text>
<rect x="200" y="118" width="130" height="52" rx="8" fill="#f6f8fa" stroke="#bc4c00"/>
<text x="265" y="138" text-anchor="middle" font-size="13" font-weight="600" fill="#bc4c00">/loop/a</text>
<text x="265" y="156" text-anchor="middle" font-size="11" fill="#57606a">乒乓节点 A</text>
<rect x="400" y="118" width="130" height="52" rx="8" fill="#f6f8fa" stroke="#bc4c00"/>
<text x="465" y="138" text-anchor="middle" font-size="13" font-weight="600" fill="#bc4c00">/loop/b</text>
<text x="465" y="156" text-anchor="middle" font-size="11" fill="#57606a">乒乓节点 B</text>
<text x="365" y="106" text-anchor="middle" font-size="11" fill="#bc4c00">A↔B 互指 · URL 完全重复</text>
<line x1="330" y1="136" x2="394" y2="136" stroke="#57606a" stroke-width="1.5" marker-end="url(#loopArrL)"/>
<line x1="400" y1="154" x2="336" y2="154" stroke="#57606a" stroke-width="1.5" marker-end="url(#loopArrL)"/>
<rect x="18" y="118" width="150" height="52" rx="8" fill="#f6f8fa" stroke="#57606a" stroke-width="0.8"/>
<text x="93" y="138" text-anchor="middle" font-size="12" font-weight="600" fill="#1f2328">服务端计数+逐跳捕获</text>
<text x="93" y="156" text-anchor="middle" font-size="11" fill="#57606a">方法·体长·CT·SHA</text>
<line x1="168" y1="144" x2="194" y2="144" stroke="#57606a" stroke-width="0.8" stroke-dasharray="4 3"/>
<polyline points="354,70 354,92 265,92 265,112" fill="none" stroke="#57606a" stroke-width="1.5" marker-end="url(#loopArrL)"/>
<text x="372" y="88" font-size="11" fill="#57606a">2 首跳</text>
<line x1="265" y1="170" x2="265" y2="204" stroke="#57606a" stroke-width="1.5" marker-end="url(#loopArrL)"/>
<text x="277" y="192" font-size="11" fill="#57606a">3 达到 limit</text>
<rect x="200" y="208" width="330" height="44" rx="8" fill="#f6f8fa" stroke="#1a7f37"/>
<text x="365" y="226" text-anchor="middle" font-size="13" font-weight="600" fill="#1a7f37">200 JSON 终止 · 服务端熔断</text>
<text x="365" y="243" text-anchor="middle" font-size="11" fill="#57606a">观测: 终端逐跳JSON · 日志时间线 · 会话进度条/逐跳明细</text>
</svg>"""

LOOP_PAGE = r"""<!doctype html><html lang="zh-CN"><meta charset="utf-8">
<title>有界循环重定向实验</title>
<style>body{max-width:880px;margin:40px auto;font:17px sans-serif;line-height:1.8;padding:0 20px;color:#1f2328}input,select,button{font:inherit;margin:6px}code{overflow-wrap:anywhere;background:#f6f8fa;padding:1px 5px;border-radius:4px}fieldset{border:1px solid #d0d7de;border-radius:8px;margin:14px 0}legend{font-size:14px;color:#57606a}</style>
<h1>有界循环重定向实验 <small style="color:#888">(v2.7)</small></h1>
<p>所有跳转仅发生在本站。默认 12 次后停止，最多 40 次；会话五分钟过期。</p>
""" + LOOP_FLOW_SVG_LIGHT + r"""
<form action="/loop/start" method="get">
<fieldset><legend>跳转方式 mode (meta/js 用于鉴别渲染型客户端)</legend>
<label><input type="radio" name="mode" value="http" checked> HTTP 30x 状态码</label>
<label><input type="radio" name="mode" value="meta"> Meta 刷新 (200+HTML)</label>
<label><input type="radio" name="mode" value="js"> JS 跳转 (200+Script)</label></fieldset>
<label>状态码 (仅 http 方式生效) <select name="code"><option value="300">300 · 多选 (通常不自动跟随)</option><option value="301">301 · 永久 (POST 常改 GET)</option><option value="302" selected>302 · 临时 (POST 常改 GET)</option><option value="303">303 · 强制改写 GET</option><option value="305">305 · 代理 (现代客户端忽略)</option><option value="307">307 · 保留方法与请求体</option><option value="308">308 · 永久+保留方法与请求体</option></select></label><br>
<label>重定向次数 <input name="limit" type="number" min="1" max="40" value="12"></label><br>
<label>测试标记 <input name="tag" maxlength="64" value="browser-test"></label><br>
<button>由浏览器执行测试</button></form>
<p>服务端抓取测试地址：<code>/loop/start?limit=12&amp;code=302&amp;mode=http&amp;tag=server-test</code></p>
<p>正常响应对照：<a href="/loop/ok">/loop/ok</a>。逐跳日志输出在服务端终端，并同步进入 Web 控制台日志时间线；控制台「循环实验」Tab 的会话明细可展开逐跳捕获 (方法 / 体长 / Content-Type / 体SHA / 预览)。</p>
<p><b>盲SSRF验证用法：</b>将目标 SSRF 点指向上述地址，即使无回显，日志即为带外证据；用 <code>tag</code> 区分不同注入点；用 307/308 验证 POST 方法与请求体能否穿透重定向到达内网目标；用 <code>mode=meta</code> / <code>mode=js</code> 鉴别目标是否为渲染型客户端 (爬虫 / 无头浏览器)。</p>
</html>"""


def _loop_int_arg(name, default, lower, upper):
    """解析并校验整数实验参数: 拒绝重复参数与越界值"""
    values = request.args.getlist(name)
    if len(values) > 1:
        raise ValueError('Duplicate parameter: ' + name)
    value = int(values[0]) if values else default
    if not lower <= value <= upper:
        raise ValueError('%s must be between %d and %d' % (name, lower, upper))
    return value


def _loop_headers(response):
    """循环实验响应统一附加安全头 (主程序无全局 after_request, 逐响应设置)"""
...
@app.route('/loop/ok', methods=SUPPORTED_METHODS)
def loop_baseline():
    """基线对照端点: 验证客户端能正常抓取普通 (非重定向) URL"""
    return _loop_headers(jsonify(ok=True, module='loop', method=request.method))


@app.route('/loop/start', methods=SUPPORTED_METHODS)
def loop_start():
    """创建实验会话并发出第一跳 (phase='start' → 首跳指向 /loop/a)"""
    try:
        limit = _loop_int_arg('limit', 12, 1, LOOP_MAX_LIMIT)
        status = _loop_int_arg('code', 302, 300, 399)
        if status not in LOOP_ALLOWED_CODES:
            raise ValueError('code must be one of ' + ', '.join(str(c) for c in LOOP_ALLOWED_CODES))
        mode = request.args.get('mode', 'http')
        if mode not in LOOP_ALLOWED_MODES:
            raise ValueError('mode must be http, meta or js')
        tag = request.args.get('tag', '')
        if not re.fullmatch(r'[A-Za-z0-9_-]{0,64}', tag):
            raise ValueError('tag must contain at most 64 letters, digits, _ or -')
    except ValueError as exc:
        log_entry = log_request()
        log_entry['status_code'] = 400
        log_entry['redirect_url'] = '[loop 实验参数校验失败: %s]' % exc
        return _loop_headers(make_response(jsonify(error=str(exc)), 400))

    token = secrets.token_hex(16)
    now = time.monotonic()
    with LOOP_LOCK:
        # 惰性回收过期会话
        for key in list(LOOP_SESSIONS):
            if LOOP_SESSIONS[key]['expires'] <= now:
                del LOOP_SESSIONS[key]
        if len(LOOP_SESSIONS) >= LOOP_MAX_SESSIONS:
            log_entry = log_request()
            log_entry['status_code'] = 503
            log_entry['redirect_url'] = '[loop 会话容量已满, 拒绝新实验]'
            return _loop_headers(make_response(jsonify(error='Loop session capacity reached; retry later'), 503))
        LOOP_SESSIONS[token] = dict(limit=limit, status=status, mode=mode, tag=tag,
                                    redirects=0, expires=now + LOOP_TTL,
                                    hops=[], last_method=None)
    return _loop_advance(token, 'start')


@app.route('/loop/a', methods=SUPPORTED_METHODS)
@app.route('/loop/b', methods=SUPPORTED_METHODS)
def loop_hop():
    """乒乓跳转入口: /loop/a 与 /loop/b 交替互指"""
    return _loop_advance(request.args.get('id', ''), request.path.rsplit('/', 1)[-1])


def _loop_capture_hop(session):
    """v2.7: 逐跳请求捕获 —— 记录 方法/路径/Content-Type/体长/体SHA/预览 (供控制台明细展开)
    请求体预览仅存会话内存, 不落终端日志; 超出 LOOP_MAX_HOPS_CAPTURED 后丢弃"""
    if len(session['hops']) >= LOOP_MAX_HOPS_CAPTURED:
        return
    body = request.get_data(cache=True) or b''
    hop = {
        'n': len(session['hops']) + 1,
        'time': get_beijing_time(),
        'method': request.method,
        'path': request.path,
    ...
    if body:
        hop['bsha'] = hashlib.sha256(body).hexdigest()[:12]
        hop['bprev'] = body[:512].decode('utf-8', 'replace')[:LOOP_BODY_PREVIEW]
    session['hops'].append(hop)


def _loop_next_response(session, token, location):
    """v2.7: 按跳转方式构造下一跳响应, 返回 (response, 实际状态码)
    http → 30x + Location; meta → 200 + <meta refresh>; js → 200 + <script>location.replace"""
    mode = session['mode']
    if mode in ('meta', 'js'):
        esc = str(html_escape(location))  # markupsafe.escape 默认转义引号, 无 quote 关键字
        if mode == 'meta':
            page = ('<!doctype html><html lang="zh-CN"><meta charset="utf-8">'
                    '<meta http-equiv="refresh" content="0;url=' + esc + '">'
                    '<title>loop meta refresh</title>'
                    '<body style="font:14px sans-serif">meta refresh → <a href="' + esc + '">' + esc + '</a></body></html>')
        else:
            page = ('<!doctype html><html lang="zh-CN"><meta charset="utf-8">'
                    '<title>loop js redirect</title>'
                    '<body style="font:14px sans-serif">js redirect → <a href="' + esc + '">' + esc + '</a>'
                    '<script>location.replace(' + json.dumps(location) + ');</script></body></html>')
        response = make_response(page, 200)
        response.headers['Content-Type'] = 'text/html; charset=utf-8'
        return response, 200
    response = make_response(jsonify(test_id=token, next=location), session['status'])
    response.headers['Location'] = location
    return response, session['status']


def _loop_advance(token, phase):
    """循环核心: 服务端计数 + 逐跳捕获, 到达 limit 后返回 200 JSON 终止 (服务端熔断)"""
    log_entry = log_request()

    # OPTIONS 探测不消耗计数、不进捕获, 避免污染方法保留实验数据
    if request.method == 'OPTIONS':
        log_entry['status_code'] = 200
        log_entry['redirect_url'] = '[loop OPTIONS 探测 → 200+Allow, 不消耗计数]'
        response = make_response('', 200)
        response.headers['Allow'] = ', '.join(SUPPORTED_METHODS)
        return _loop_headers(response)

    with LOOP_LOCK:
        session = LOOP_SESSIONS.get(token)
        if session is None or session['expires'] <= time.monotonic():
            log_entry['status_code'] = 410
            log_entry['redirect_url'] = '[loop 未知或过期的实验会话 → 410]'
            return _loop_headers(make_response(jsonify(error='Unknown or expired loop session'), 410))
        done = session['redirects'] >= session['limit']
        session['last_method'] = request.method  # 供控制台会话列表实时展示
        _loop_capture_hop(session)               # v2.7: 逐跳捕获 (含首跳与熔断末跳)
        if not done:
            session['redirects'] += 1
        count = session['redirects']
        mode = session['mode']
        tag = session['tag']
        # 终端逐跳 JSON 日志 (不含请求体/Cookie/Authorization)
        print(json.dumps({
            'time': get_beijing_time(), 'module': 'loop', 'test_id': token,
            'tag': tag, 'mode': mode, 'peer': request.remote_addr, 'method': request.method,
            'path': request.path, 'redirects': count,
            'status': (200 if (done or mode != 'http') else session['status']),
            'event': 'finished' if done else 'redirect'
        }, ensure_ascii=False), flush=True)

    if done:
        log_entry['status_code'] = 200
        log_entry['redirect_url'] = '[loop 实验完成: %d 次重定向, 最终方法 %s]' % (count, request.method)
        log_entry['extra_data'] = {
            'loop_finished': True, 'test_id': token, 'tag': tag, 'mode': mode,
            'redirects': count, 'final_method': request.method
        }
        return _loop_headers(jsonify(message='Server test limit reached', test_id=token,
                                      redirects=count, final_method=request.method))

    next_phase = 'b' if phase == 'a' else 'a'
    location = '/loop/' + next_phase + '?' + _loop_urlencode({'id': token})
    response, out_status = _loop_next_response(session, token, location)
    log_entry['status_code'] = out_status
    log_entry['redirect_url'] = location
    log_entry['extra_data'] = {
        'loop_redirect': True, 'test_id': token, 'tag': tag, 'mode': mode, 'redirects': count
    }
    return _loop_headers(response)

# ==================== v2.7: 循环实验会话管理 API (挂载于控制台随机路径之下) ====================
@app.route(CONSOLE_PATH + '/api/loop_sessions', methods=['GET'])
def list_loop_sessions():
    """列出活跃的循环实验会话 (供控制台「循环实验」标签页实时观测带外证据 + 逐跳捕获明细)"""
    if not check_access_token():
        return jsonify({'error': 'Unauthorized'}), 401
    now = time.monotonic()
    with LOOP_LOCK:
        ...
        sessions = [{
            ...
            'limit': s['limit'],
            'status': s['status'],
            'mode': s.get('mode', 'http'),
            'tag': s['tag'],
            'redirects': s['redirects'],
            'last_method': s.get('last_method'),
            'hops': s.get('hops', []),
            'ttl_remaining': max(0, int(s['expires'] - now)),
        } for token, s in LOOP_SESSIONS.items()]
    sessions.sort(key=lambda x: -x['ttl_remaining'])
    return jsonify({'sessions': sessions, 'total': len(sessions)})

@app.route(CONSOLE_PATH + '/api/loop_sessions/<token>', methods=['DELETE'])
def delete_loop_session(token):
    """终止指定循环实验会话 (客户端后续跳转将收到 410)"""
    if not check_access_token():
        return jsonify({'error': 'Unauthorized'}), 401
    if not re.fullmatch(r'[0-9a-f]{32}', token or ''):
        return jsonify({'error': 'Invalid token'}), 400
    with LOOP_LOCK:
        if token not in LOOP_SESSIONS:
            return jsonify({'error': 'Session not found or expired'}), 404
        del LOOP_SESSIONS[token]
    return jsonify({'message': 'Loop session terminated', 'remaining': len(LOOP_SESSIONS)})
...
def run_startup_wizard():
    ...
    print('=' * 62)
    print('  ⚡ SSRF 30x Redirector v2.6 · 启动配置向导')
    print('=' * 62)
...
if __name__ == '__main__':
    ...
    print(f"\n{'=' * 62}")
    print(f"🚀 SSRF 实战 30x Redirector 引擎启动成功 (v2.6)")
    print(f"{'=' * 62}")
    ...
    print(f"🔁 有界循环重定向实验已启用 (v2.7): /loop 同源A<->B乒乓循环 | 三种跳转方式(http 30x/meta刷新/js跳转) | 7档状态码(300/301/302/303/305/307/308) | 逐跳请求捕获(方法/体长/CT/体SHA) | 1-{LOOP_MAX_LIMIT}次服务端熔断")
    print(f"🖥 循环实验控制台 (v2.7): 内嵌流程原理图 | 高级请求编辑(自定义首跳方法/请求头/请求体 → 一键curl/浏览器直发) | 活跃会话逐跳明细实时监控/终止")
