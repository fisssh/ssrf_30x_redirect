...
<body>
            ...
            <div class="card">
                ...
                <div id="tab-loop" class="tab-content">
                    <div class="protocol-tip tip-orange">
                        ...
                        ① 是否跟随 30x; ② 实际跟随几跳 (对比客户端 max_redirects 上限); ③ <b>302/303 是否将 POST 改写为 GET, 307/308 是否保留方法与请求体</b> (SSRF 打内网 API 的关键前提);
                        ...
                        即使无回显, 右侧日志时间线与下方会话列表即为带外证据; 用 307+POST 可验证请求体能否穿透重定向到达内网目标。
                    </div>
                    ...
                    <div class="bb-flow-steps">
                        <div class="bb-flow-step">
                            <div class="bb-flow-num">1</div>
                            <div><b>配置实验参数</b><span>状态码 (方法保留实验) / 重定向次数 (服务端熔断线) / 测试标记</span></div>
                        </div>
                        <div class="bb-flow-step">
                            <div class="bb-flow-num">2</div>
                            <div><b>指向 SSRF 注入点</b><span>将生成的 /loop/start 地址作为 payload, 或用浏览器直接打开实验控制页</span></div>
                        </div>
                        <div class="bb-flow-step">
                            <div class="bb-flow-num">3</div>
                            <div><b>观测逐跳行为</b><span>日志时间线 / 活跃会话列表: 跟随跳数 · 最终方法 · 循环检测 · 熔断终止</span></div>
                        </div>
                    </div>
                    ...
                    <div class="bb-create-grid four">
                        <div>
                            <label>状态码 (方法保留实验)</label>
                            <select id="loopCode">
                                <option value="302" selected>302 · POST 可能被改写 GET</option>
                                <option value="303">303 · 强制改写为 GET</option>
                                <option value="307">307 · 保留方法与请求体</option>
                                <option value="308">308 · 保留方法与请求体</option>
                            </select>
                        </div>
                        <div>
                        ...
                        <div>
                            <label>&nbsp;</label>
                            <button class="btn" onclick="generateLoopPayload()" style="width:100%;margin:0;">🔁 生成实验 Payload</button>
                        </div>
                    </div>
                    ...
                    <div class="chain-manager" style="margin-top:18px;">
                        ...
                        <div style="font-size:12px;color:var(--text-secondary);margin-bottom:12px;line-height:1.7;">
                            ...
                            盲SSRF场景下, 进度条增长即目标服务器正在跟随重定向的带外证据。
                        </div>
    ...
    <script>
        ...
        // ========== v2.6: 有界循环重定向实验 (集成至 Payload 构建控制台) ==========
        function generateLoopPayload() {
            ...
            if (isNaN(limit) || limit < 1 || limit > 40) return alert('⚠️ 重定向次数必须为 1-40 的整数 (服务端熔断线)');
            if (tag && !/^[A-Za-z0-9_-]{1,64}$/.test(tag)) return alert('⚠️ 测试标记 tag 仅支持 字母/数字/_/-, 最长 64 字符');
            let url = `${BASE_URL}/loop/start?limit=${limit}&code=${code}`;
        ...
        async function loadLoopSessions() {
            try {
                ...
                container.innerHTML = sessions.map(s => {
                    ...
                    const startUrl = `${BASE_URL}/loop/start?limit=${s.limit}&code=${s.status}${s.tag ? '&tag=' + encodeURIComponent(s.tag) : ''}`;
                    ...
                    <div class="chain-item">
                        <div class="chain-item-header">
                            <span class="chain-item-id">🔁 ${escapeHtml(String(s.token_short))} <span style="background:rgba(240,136,62,0.14);color:var(--accent-orange);padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;margin-left:6px;">${s.status}</span>${doneBadge}</span>
...
# ==================== v2.5: 有界循环重定向实验模块 (Redirect Loop Lab) ====================
# 整合自独立实验脚本 redirect_loop_lab.py —— 同源 A<->B 有界循环重定向实验环境
#
# 端点一览:
#   GET  /loop            实验控制页 (浏览器测试入口)
#   ANY  /loop/start      创建实验会话并发出第一跳  ?limit=12&code=302&tag=xxx
...
#   3. 302/303 是否将 POST 改写为 GET, 307/308 是否保留方法与请求体 (SSRF 打内网 API 的关键前提);
...
#     终端 JSON 不含请求体/Cookie/Authorization, 控制台条目遵循主程序既定记录规则;
...
LOOP_SESSIONS = {}            # token -> {limit, status, tag, redirects, expires}
LOOP_TTL = 300                # 实验会话有效期 (秒)
LOOP_MAX_SESSIONS = 1000      # 并发会话上限 (防自身被反向DoS)
LOOP_MAX_LIMIT = 40           # 单次实验最大重定向次数 (服务端熔断线)
LOOP_ALLOWED_CODES = (302, 303, 307, 308)

LOOP_PAGE = r"""<!doctype html><html lang="zh-CN"><meta charset="utf-8">
<title>有界循环重定向实验</title>
<style>body{max-width:850px;margin:40px auto;font:17px sans-serif;line-height:1.8;padding:0 20px}input,select,button{font:inherit;margin:6px}code{overflow-wrap:anywhere}</style>
<h1>有界循环重定向实验 <small style="color:#888">(v2.5 集成模块)</small></h1>
<p>所有跳转仅发生在本站。默认 12 次后停止，最多 40 次；会话五分钟过期。</p>
<form action="/loop/start" method="get">
<label>状态码 <select name="code"><option>302</option><option>303</option><option>307</option><option>308</option></select></label><br>
...
<p>服务端抓取测试地址：<code>/loop/start?limit=12&amp;code=302&amp;tag=server-test</code></p>
<p>正常响应对照：<a href="/loop/ok">/loop/ok</a>。逐跳日志输出在服务端终端，并同步进入 Web 控制台日志时间线。</p>
<p><b>盲SSRF验证用法：</b>将目标 SSRF 点指向上述地址，即使无回显，日志即为带外证据；用 <code>tag</code> 区分不同注入点；用 307/308 验证 POST 方法与请求体能否穿透重定向到达内网目标。</p>
</html>"""
...
def loop_start():
    ...
    try:
        ...
        if status not in LOOP_ALLOWED_CODES:
            raise ValueError('code must be 302, 303, 307 or 308')
    ...
    with LOOP_LOCK:
        ...
        LOOP_SESSIONS[token] = dict(limit=limit, status=status, tag=tag,
...
def _loop_advance(token, phase):
    """循环核心: 服务端计数, 到达 limit 后返回 200 JSON 终止 (服务端熔断)"""
    log_entry = log_request()
    ...
    with LOOP_LOCK:
        ...
        session['last_method'] = request.method  # v2.6: 供控制台会话列表实时展示
        if not done:
            session['redirects'] += 1
        count = session['redirects']
        status = 200 if done else session['status']
        tag = session['tag']
        # 终端逐跳 JSON 日志 (不含请求体/Cookie/Authorization)
        print(json.dumps({
            'time': get_beijing_time(), 'module': 'loop', 'test_id': token,
            'tag': tag, 'peer': request.remote_addr, 'method': request.method,
            'path': request.path, 'redirects': count, 'status': status,
    ...
    if done:
        ...
        log_entry['extra_data'] = {
            'loop_finished': True, 'test_id': token, 'tag': tag,
    ...
    response = make_response(jsonify(test_id=token, next=location), status)
    response.headers['Location'] = location
    log_entry['status_code'] = status
    log_entry['redirect_url'] = location
    log_entry['extra_data'] = {
        'loop_redirect': True, 'test_id': token, 'tag': tag, 'redirects': count
    }
    return _loop_headers(response)

# ==================== v2.6: 循环实验会话管理 API (挂载于控制台随机路径之下) ====================
@app.route(CONSOLE_PATH + '/api/loop_sessions', methods=['GET'])
def list_loop_sessions():
    """列出活跃的循环实验会话 (供控制台「循环实验」标签页实时观测带外证据)"""
    if not check_access_token():
        return jsonify({'error': 'Unauthorized'}), 401
    now = time.monotonic()
    with LOOP_LOCK:
...
if __name__ == '__main__':
    ...
    print(f"🔁 有界循环重定向实验已启用 (v2.5): /loop 同源A<->B乒乓循环 | 302/303/307/308方法保留验证 | 1-{LOOP_MAX_LIMIT}次服务端熔断 | 逐跳日志进入控制台时间线")
    print(f"🖥 循环实验已集成至 Payload 构建控制台 (v2.6): 与 基础跳转/智能链/有状态链/盲SSRF基准 并列 | 活跃会话实时监控/终止")
